...
class ReflexCaptureAgent(CaptureAgent):
    ...
    def choose_action(self, game_state):
        ...
        food_self = len(self.get_food_you_are_defending(game_state).as_list())
        food_rival = food_left-food_self
        if food_left <= 10:
            best_dist = 9999
            best_action = None
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  """
    ...
    def choose_action(self, game_state):
        ...
        food_left = len(self.get_food(game_state).as_list())
        food_self = len(self.get_food_you_are_defending(game_state).as_list())
        if food_left < food_self-2 and  game_state.get_agent_state(self.index).is_pacman:
            best_dist = 9999
            best_action = None
            for action in actions:
                successor = self.get_successor(game_state, action)
    ...
    def get_features(self, game_state, action):
        ...
        if food_self-2>food_left:
            features['on_defense'] = 1
        else:
            features['on_defense'] = 0
        
       
        if len(food_list) > 0 and features['on_defense']==0:  # This should always be True,  but better safe than sorry
            ...
            is_food=False
            features["no_move"] = 0
            while len(actual_state.get_legal_actions(self.index))==3:
                if len(actual_state.get_legal_actions(self.index))!=3:break
                for food in food_list:
                    if food==actual_state.get_agent_state(self.index).get_position():
                        is_food=True
                        break
                for action in actual_state.get_legal_actions(self.index):
                    if inverse_dir(action)!="Stop" and inverse_dir(action)!=previous_action: 
                        previous_action=action
                        break
                
                actual_state=self.get_successor(actual_state,previous_action)
                movecount+=1
            ...
            if(successor.get_agent_state(self.index).get_position()==game_state.get_agent_state(self.index).get_position()): features["static"] =1
            else: features["static"] =0
        else:
            features = util.Counter()
            successor = self.get_successor(game_state, action)
